...
var popularSortBtn = document.querySelector('#popularSortBtn');
var newSortBtn = document.querySelector('#newSortBtn');
var currentPageBtnNum;

var currentPage = function currentPage() {
  var pageSearch = window.location.pathname.split('/');
  var currentPage = pageSearch[pageSearch.length - 1];
  return Number(currentPage);
};

var transferPageNum = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regenerator_runtime__WEBPACK_IMPORTED_MODULE_0___default().mark(function _callee(pageNum) {
    var fetchPage;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return fetch("/community/community/".concat(pageNum));

          case 3:
            fetchPage = _context.sent;
            console.log(fetchPage.status);

            if (fetchPage.status === 200) {
              window.location.replace("/community/community/".concat(pageNum));
            }
  ...
  return function transferPageNum(_x) {
    return _ref.apply(this, arguments);
  };
}();

var handlePrePage = function handlePrePage(e) {
  e.preventDefault();
...
var handleCurrentPage = function handleCurrentPage(e) {
...
var handlePopularSort = function handlePopularSort(e) {
  var currentPath = window.location.pathname.split('/');

  if (currentPath.includes('popular')) {
    return;
  } else {
    var popularA = popularSortBtn.firstElementChild;
    popularA.href = "/community/sort/popular/".concat(currentPage());
  }
};

var handleNewSort = function handleNewSort(e) {
  var currentPath = window.location.pathname.split('/');

  if (currentPath.includes('new')) {
    return;
  } else {
    var newA = newSortBtn.firstElementChild;
    newA.href = "/community/sort/new/".concat(currentPage());
  }
};
